...
class userController extends Controller
{
 ...
 $registration=\DB::table('user_reg')->insert(
       ['rand_id' => $random_id,'username' => $username, 'password'=>$password,'email' => $email,'gender' => $gender,        
        'dob' =>$date_for,'date'=>$cur_date,'contact_num' => $contact_num,'email_key' =>null]);
   ...
   foreach($user_login_values as $values)
        {
         ...
         \Session::put(['gender'=>$gender,'username'=>$username,'id'=>$user_id]); 
   ...
   if( !empty($results) ) 
  {
    ...
    if(empty($user->religion)){$user_religion= $user->other_religion;}else {
      ...
      $response .= ' <div class="col-md-4">
                     <div class="prof-wrap">
               <div class="prof-img1"> <a href="search-profile-view/'.$encrypted_id.'"><img style="width:100%;height:240px;" src="'.$path.'"/></a></p>
               </div>
                      <div class="prof-cv">
                      <ul class="child">
                        <li>Name</li>
                      ...
                      <ul class="child">
                        <li>Religion</li>
                        <li>:&nbsp;&nbsp; '.$user_religion.'-'.$user_caste.'</li>
                      </ul>
                      <ul class="child">
                        <li>Place</li>
                        <li>:&nbsp;&nbsp;'. $user->district.','.$user->state.'</li>
                      </ul>
                    </div>
...
$intrstd_ppl_notification=\DB::table('notification')
  ...
  public function postChangeEmail()
  {
    ...
    if($count==0)
    {
           ...
           $v_email=\DB::table('settings')->get();
           foreach ($v_email as $email_value) {
            $email_verification=$email_value->email_verification;
           }
          if($email_verification=="on")
           {
