...
<form method='post' id='taxi_reg1' style="display:block;" action="">
<div class="smtpdetails">
  ...
  die("Connection error: " . mysqli_connect_error());
								...
								//  var_dump($query1);exit;
								  while( $row1=mysqli_fetch_array($query1,MYSQLI_ASSOC))
								  {
                                               ...
                                               <option value="<?php echo $row1['iso_alpha2'];?>" ><?php echo $row1['country'];?></option>
                                              <?php
													}?>
                                                  </select></td></tr>
			<tr><td>Currency</td><td><select   name="currency"  id="currency">
										   <?php
										   	$query2 = mysqli_query($con,"SELECT DISTINCT`currency_symbol`,`currency_name`,currency_code FROM `country` where `currency_symbol` IS NOT NULL ");
											
											//var_dump($query2);
                                                    
								 while( $row1=mysqli_fetch_array($query2,MYSQLI_ASSOC))
								  {
						
														
														$array["a"] = $row1['currency_code'];
                                                        $array["b"] = $row1['currency_symbol'];
														 $str = implode(',',$array);
                                                         
                                                            ?>
										  
       <option value="<?php echo $row1['currency_symbol'];?>" ><?php echo $row1['currency_symbol'];?></option>
                                              <?php
													}?>
                                                  </select></td></tr>
    	   <tr><td>Smtp Username</td><td><input type="text" name="smtp_username" id="smtp_username"  class="regcom" placeholder="Smtp Username" /></td></tr>
            ...
            <tr><td>Smtp Port</td><td><input type="text" name="smtp_port" id="smtp_port" class="regcom"  placeholder="Smtp Port" /></td></tr>
		...
		<?php
		$query = mysqli_query($con," SELECT * FROM `settings` order by id DESC ");
		//var_dump($query);
		...
		<input type="submit" name="add_floor1" id='selectadd'  value="Save Changes "  class="button button-primary"/>
</div>
...
if (isset($_POST['add_floor1']))
{
    ...
    $sett=mysqli_query($con,"UPDATE settings SET title='$title',currency='$currency',smtp_username='$smtp_username',smtp_host='$smtp_host',smtp_password='$smtp_password' WHERE id='$id'");

$myfile2 = fopen("../local/.env", "w") or die("Unable to open file!");
$txt2 = $_SESSION['env_file'];

$txt2 .= "MAIL_DRIVER=smtp"."\r\n";
$txt2 .= "MAIL_HOST='".$smtp_host."'
         MAIL_PORT='".$smtp_port."'
...
fwrite($myfile2, $txt2);
	fclose($myfile2);
 $myfile3 = fopen("../local/config/mail.php", "w") or die("Unable to open file!");
 $active_record='';
$txt = '<?php'."\r\n";
...
$txt .="'host'=> env('MAIL_HOST','". $smtp_host."'),"."\r\n";
$txt .="'port' => env('MAIL_PORT','". $smtp_port."'),"."\r\n";
$txt .='"from"=>array('."\r\n";
$txt .='"address" =>"'.$smtp_username.'",'."\r\n";
$txt .='"name" =>"Soulmate"'."\r\n";
$txt .=' ),'."\r\n";
$txt .="'encryption' => env('MAIL_ENCRYPTION','tls'),"."\r\n";
$txt .='"username"=>"'.$smtp_username.'",'."\r\n";

$txt .='"password"=>"'.$smtp_password.'",'."\r\n";
...
fclose($myfile3);
    ...
    var_dump($sett);
      if($sett)
      {
		  header("Location:../user/first-show");
